...
import confResume from "../services/resume.json";
import confThemes from "../services/themes.json";
          ...
          <section id="experience">
            ...
            <div className="content">
              {cv["work-experience"].map((e) => {
                return (
                  <Experience key={e["name"]} title={e["name"]} caption={buildDates(e["starting-date"], e["finishing-date"])} subtitle={e["company"]} description={e["missions"]} />
                );
              })}
            </div>
          </section>
              ...
              {cv["education"].map((e) => {
                return (
                  <Education
                    key={e["name"]}
                    title={e["name"]}
                    caption={buildDates(e["starting-date"], e["finishing-date"])}
              ...
              return (
                <tr key={s}>
                  <td className="subtitle">
                    <h4>{s.split("!")[0].split(" - ")[0]}</h4>
                  </td>
                  <td>
                    <p>{s.split("!")[0].split(" - ")[1]}</p>
                  </td>
                  <td className="subtitle">
                    <h4>{s.split("!")[1].split(" - ")[0]}</h4>
                  </td>
                  <td>
                    <p>{s.split("!")[1].split(" - ")[1]}</p>
                  </td>
                </tr>
              );
            }
