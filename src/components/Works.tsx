...
const Works = (props) => {
  ...
  useEffect(() => {
    ...
    const getRepos = async () => {
      octokit.request("GET /users/{user}/repos", { user: "theolemague", type: "public" }).then(async (res) => {
  ...
  return (
    <section className="works" style={{ opacity: opacity }}>
      ...
      {repos === null ? <h3>{labels["labels"]["no-works"]}</h3> : repos.length === 0 ? <h3>{labels["labels"]["loading"]}</h3> : <CardGrid repos={repos} />}
    </section>
  );
};
    ...
    if (desc.includes("](")) {
      ...
      return (
        <>
          ...
          {formatDesc(postlink.slice(postlink.indexOf(")") + 1))}
        </>
      );
    } else {
      return <>{desc}</>;
